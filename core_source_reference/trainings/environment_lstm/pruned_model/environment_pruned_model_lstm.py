...
# ---------------------
# Compile (reduced LR for stability)
# ---------------------
...
# ============================================================
# GATING IDENTICAL TO THE FIRMWARE:
...
def _metrics_like_firmware(y_true_2d: np.ndarray, y_pred_2d: np.ndarray):
    ...
    seguindo exatamente o firmware:
      mae = 0.5*(mae_T + mae_H)
...
plt.xlabel("T_in Real (°C)")
plt.ylabel("T_in Previsto (°C)")
...
plt.xlabel("H_in Real (%)")
plt.ylabel("H_in Previsto (%)")
...
metrics_dist = {
    "Pruned Model Metrics": [
        ...
        "MSE (joint original)", "RMSE (joint original)", "MAE (joint original)", "R² (joint original)",
        # Per-variable T_in
    ...
    "Valor": [
        ...
        f"{sparsity_before * 100:.2f} %", f"{sparsity_after * 100:.2f} %", f"{model_size_kb:.2f} KB",
