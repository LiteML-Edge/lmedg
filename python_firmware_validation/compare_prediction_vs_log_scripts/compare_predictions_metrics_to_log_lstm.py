...
def resolve_runtime_paths(
    ...
    if cli_pred:
        ...
        if not QUANTIZED_MODEL_METRICS:
            raise FileNotFoundError("Prediction spreadsheet path not provided and utils.paths_lstm is unavailable.")
    ...
    if cli_metrics:
        ...
        if results_dir is None:
            if not QUANTIZED_MODEL_METRICS:
                raise FileNotFoundError("Metrics spreadsheet path not provided and utils.paths_lstm is unavailable.")
            metrics_root = normalize_metrics_root(Path(QUANTIZED_MODEL_METRICS))
            latest_run_dir = find_latest_run_dir(metrics_root)
            results_dir = latest_run_dir / "quantization_metrics_results"
    ...
    if cli_log:
        ...
        if not PROJECT_ROOT:
            raise FileNotFoundError("Log path not provided and utils.paths_lstm is unavailable.")
        
    workbook_dir = (Path(PROJECT_ROOT) / "utils" / "workbook_lstm")

    log_dir = Path(PROJECT_ROOT) / "firmwares" / "environment_lstm" / "PlatfIO_ESP32_Wemos_lstm" / "logs"
    log_path = find_latest_log_or_zip(log_dir)

    if cli_out:
        out_path = choose_writable_output_path(Path(cli_out))
    else:
        preferred = workbook_dir / "predictions_metrics_vs_log_comparison_lstm.xlsx"
...
def add_executive_summary_sheet(
    ...
    ws["A2"] = "MODEL: LSTM -> Python vs Firmware Comparison"
